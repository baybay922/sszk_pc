<template>
	<section>
      ...
      <el-table-column prop="imgUrl" label="营业执照" >
        <template scope="scope">
          <a v-bind:href="scope.row.imgUrl" target="blank">{{scope.row.imgUrl?'查看照片':''}}</a>
        </template>
			</el-table-column>
      <el-table-column prop="bankNumber" label="银行卡号">
			</el-table-column>
      <el-table-column prop="bankName" label="持卡人姓名">
			</el-table-column>
      <el-table-column prop="bankIdcard" label="身份证号">
			</el-table-column>
...
<script>
	...
	import { getUbLatentList, checkPass,checkBLevel,lockAccount } from '../../api/api';
	...
	export default {
      ...
      showpasswindow(orgId) {
        ...
        this.listLoading = true;
				getUbLatentList(para).then((res) => {
          console.log(res,'待完善企业渠道-------------')
