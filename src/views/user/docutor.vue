<template>
	<section>
		...
		<el-table :data="users" highlight-current-row v-loading="listLoading" style="width: 100%;">
			<el-table-column prop="orgName" label="姓名"></el-table-column>
      <el-table-column prop="leaderPhone" label="手机号"></el-table-column>
      <el-table-column label="地区">
				<template scope="scope">
					<p>{{scope.row.province}}{{scope.row.city?"-"+scope.row.city:""}}</p>
				</template>
			</el-table-column>
      <el-table-column label="资质照片">
				<template scope="scope">
					<el-button @click="showImg(scope.row.orgImgUrls)" :disabled="scope.row.orgImgUrls == null?true:false">查看图片</el-button>
				</template>
			</el-table-column>
...
import { doctorPerfect, supplierCheck, supplierauditre } from "../../api/api";
...
export default {
  ...
  methods: {
    ...
    getUsers() {
      ...
      doctorPerfect(para).then(res => {
        console.log(res,'待完善医生-----------------')
